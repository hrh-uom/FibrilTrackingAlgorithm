...
def errorthresh(a, b, c, skip, reduction=0): #max values for error cutoff.
 return (1/(a+b+c))*(a *skip + b + c )*(1+reduction)
...
def fibril_mapping(a,b,c, MC, FR_local, skip=1, reduction=0, rAnge=lastplane_tomap(junk)):
    ...
    for pID in range (lastplane_tomap(junk)):
        ...
        delete=np.concatenate((np.where(sort_errs==0)[0],np.where(sort_errs>errorthresh(a,b,c, skip, reduction))[0]), axis=0)
...
def make_biscuit_morph_comp(MC_local, FR):
    ...
    for pID in range (nplanes):
        ...
        tracked_obj=FR[:,pID][FR[:,pID]>-1]
        tracked_indices=np.array(np.unravel_index(np.where(tracked_obj[:,None]+1== MC_local[pID].ravel())[1], (npix,npix))).T #https://stackoverflow.com/questions/49987552/numpy-where-used-with-list-of-values
        for k in range (len(tracked_indices)):
            #effectively deleting mapped fibrils, leaving us with the leftovers
            MC_local[pID, tracked_indices[k,0], tracked_indices[k,1]]=0
    return MC_local
MC2=make_biscuit_morph_comp(MC, FR_core)

#%%
md.animation_inline(MC2,np.arange(1), FR_core,0,nplanes)


##%% RUNNING ON BISCUIT leftovers

FR_extra=initialise_fibril_record(MC2)

FR_extra.shape


FR_extra=fibril_mapping(a, b, c, MC2,FR_extra, reduction=5)
FR_extra.shape
...
md.animation_inline(MC2,np.arange(FR_extra.shape[0]), FR_extra,0,nplanes)
md.beep()
