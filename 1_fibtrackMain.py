...
def ndropped(a, b, c):
    ...
    fib_rec=np.full((nfibs,nplanes),-1, dtype=int); #-1 means no fibril here, as indices are >= 0
    fib_rec[:,0]=np.arange(nfibs); #use like fib_rec[fID, pID]
    pID=0; #starting at the top  #LOOP OVER PLANES HERE
    dz_b, dz_f=increments_back_forward(pID,junk)
    err_table=np.zeros([nfibs,np.max(morphComp[pID+dz_f])]); #table of errors i,j. Overwritten at each new pID
    ...
    #delete pairs with 0 errors (ie those who are outside the box) and those above the threshold
    delete_errs=np.concatenate((np.where(sort_errs==0)[0],np.where(sort_errs>errorthresh(a,b,c))[0]), axis=0)
    sort_err_pairs=np.delete(sort_err_pairs,delete_errs, axis=0)
    ...
    return (nfibs/np.count_nonzero(fib_rec[:,1]<0));
...
fun_map = np.empty((a.size, b.size))
for i in range(a.size):
    for j in range(b.size):
        fun_map[i,j] = ndropped(a[i], b[j],c)

md.beep();
#%%
fig = plt.figure()
s = fig.add_subplot(1, 1, 1, xlabel='a', ylabel='b')
extent = [np.log10( a[0]), np.log10( a[-1]), np.log10( b[0]), np.log10( b[-1])];
im = plt.imshow(fun_map, extent=extent,origin='lower')
fig.colorbar(im);
plt.title('1 in how many fibrils dropped. Axes are powers of 10, c=1')
plt.show()
